...
async def get_alert_notifications(
    user_id: Optional[str] = Query(None, description="Filter by user ID"),
    alert_rule_id: Optional[str] = Query(None, description="Filter by alert rule ID"),
    ...
    return [
        AlertNotificationOut(
            ...
            deliveredAt=notification.deliveredAt.isoformat() if notification.deliveredAt else None,
...
async def trigger_alert_rule(rule_id: str, session: AsyncSession = Depends(get_db)):
    ...
    return {"message": "Alert rule triggered successfully", "triggerCount": rule.triggerCount + 1}
